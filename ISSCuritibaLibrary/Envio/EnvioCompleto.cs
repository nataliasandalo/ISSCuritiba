...
namespace NFSE.Net.Envio
{
    public class EnvioCompleto
    {
        ...
        public Core.RespostaEnvioNFSe EnviarLoteRps(Core.Empresa empresa, Core.ArquivosEnvio localArquivos)
        {
            try
            {
                ...
                while (true)
                {
                    ...
                    if (((dynamic)respostaSituacao).Items[0] is ListaMensagemRetorno)
                    {
                        if ((((dynamic)respostaSituacao).Items[0]).MensagemRetorno[0].Codigo == "E92")  //Lote ainda em processamento, tentando denovo.
                            continue;
            ...
            try
            {
                ...
                if (respostaEnvioLote != null)
                {
                    if (respostaEnvioLote.Item is ListaMensagemRetorno)
                    {
                        ...
                        retorno.CodigoErro = ((dynamic)mensagensErro).MensagemRetorno[0].Codigo;
                        retorno.Correcao = ((dynamic)mensagensErro).MensagemRetorno[0].Correcao;
                        retorno.MensagemErro = ((dynamic)mensagensErro).MensagemRetorno[0].Mensagem;
                        return retorno;
                    }
                    else
                    {
                        ...
                        retorno.NumeroNfse = ((dynamic)cancelamento).Pedido.InfPedidoCancelamento.IdentificacaoNfse.Numero;
                        return retorno;
                    }
                }
                else
        ...
        private ConsultarSituacaoLoteRpsResposta ConsultarSituacaoLote(Core.Empresa empresa, EnviarLoteRpsResposta protocolo, Core.ArquivosEnvio localArquivos)
        {
            ...
            consultaSituacaoLote.Protocolo = ((dynamic)protocolo).Items[2].ToString();
        ...
        private Core.RespostaEnvioNFSe MontarResposta(EnviarLoteRpsEnvio lote, ListaMensagemRetorno listaRetorno, ConsultarLoteRpsRespostaListaNfse respostaConsulta, ConsultarNfseRpsResposta respostaRps)
        {
            ...
            int indice = 0;
            foreach (var item in ((dynamic)lote).LoteRps.ListaRps)
            {
                var resp = new Core.ItemResposta
                {
                    LoteEnvio = ((dynamic)lote).LoteRps.NumeroLote,
                    NumeroRps = item.InfRps.IdentificacaoRps.Numero,
                    Serie = item.InfRps.IdentificacaoRps.Serie,
                    Identificacao = item.InfRps.id
                };
                ...
                {
                    resp.Sucesso = true;
                    foreach (var itemComplNfe in respostaConsulta.ComplNfse)
                    {
                        resp.IdentificacaoRetorno = itemComplNfe.Nfse.InfNfse.CodigoVerificacao;
                        resp.UrlConsulta = itemComplNfe.Nfse.InfNfse.OutrasInformacoes;
                    }
                ...
                else if (respostaRps != null)
                {
                    ...
                    foreach (var itemComplNfeaux in respostaConsulta.ComplNfse)
                    {
                        resp.Identificacao = itemComplNfeaux.Nfse.InfNfse.CodigoVerificacao;
                        resp.UrlConsulta = itemComplNfeaux.Nfse.InfNfse.OutrasInformacoes;
                    }
